...
public class ServerAPI {
    ...
    public void getUserInfo(final String username,Context context){
        String url = makeURL("get-contact-info",username);
        Log.d(LOG,"getting user info with "+url);
        final Context new_context = context;
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            final String name = response.getString("username");
                            final String image = response.getString("image");
                            final String key = response.getString("key");
                            final DBHandler_C db = new DBHandler_C(new_context);
    ...
    public void registerContacts(String username, final ArrayList<String> names, final Context context){
                            ...
                            try {
                                ...
                                for(String friend : names){
                                    if(status.getString(friend).equals("logged-in")){
                                        sendContactLogin(friend,context);
                                        final DBHandler_C db = new DBHandler_C(new_context);
                                        db.updatecontactstatus(friend,1);

                                    } else {
                                        sendContactLogout(friend,context);
                                        final DBHandler_C db = new DBHandler_C(new_context);
                                        db.updatecontactstatus(friend,0);
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
     ...
     */
    public void startPushListener(final String username, Context context){
        ...
        JsonObjectRequest jsObjRequest = new JsonObjectRequest(Request.Method.GET, url, null,
                new Response.Listener<JSONObject>() {
                    ...
                    @Override
                    public void onResponse(JSONObject response) {
                        handleNotifications(response, new_context);
                        try {
                            Thread.sleep(1000);
    ...
    private void handleNotification(JSONObject notification,Context context){
        try {
            String type = notification.getString("type");
            if(type.equals("login")){
                sendContactLogin(notification.getString("username"),context);
            }
            if(type.equals("logout")){
                sendContactLogout(notification.getString("username"),context);
            }
            if(type.equals("message")){
                handleMessage(notification.getJSONObject("content"),context);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    private void handleMessage(JSONObject message,Context context){
        ...
        try{
            ...
            final DBHandler db = new DBHandler(new_context);
    ...
    private void handleNotifications(JSONObject notifications, Context context){
        try {
            JSONArray array = notifications.getJSONArray("notifications");
            for(int index = 0; index < array.length(); index++){
                handleNotification(array.getJSONObject(index),context);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    private void sendContactLogin(String username,Context context){
        for(Listener listener : myListeners){
            listener.onContactLogin(username);
        }

        final DBHandler_C db = new DBHandler_C(context);
        db.updatecontactstatus(username,1);
    }
    private void sendContactLogout(String username,Context context){
        for(Listener listener : myListeners){
            listener.onContactLogout(username);
        }
        final DBHandler_C db = new DBHandler_C(context);
        db.updatecontactstatus(username,0);
    }
